...
var Config = class _Config {
  ...
  static get defaults() {
    return {
      ...
      USER_PROXY: "193.123.90.82:12648",
      // "host[:port],domain.com,...", 443 port can ignore
      LOG_LEVEL: "none",
      // debug, info, error, none
      TIME_ZONE: "8",
      // timestamp time zone of logs
      BUFFER_SIZE: "64",
      // Upload/Download buffer size in KiB
      IP_QUERY_PATH: "/ip",
      // URL path for querying client IP information, empty means disabled
      WS_PATH: "/8640655f/ws",
      // URL path for ws transport, e.g. '/ws', empty means disabled
      XHTTP_PATH: "/8640655f/xhttp",
      // URL path for xhttp transport, e.g. '/xhttp', empty means disabled
      XPADDING_RANGE: "100-1000",
      // Length range of X-Padding response header
  ...
  constructor(env = {}) {
    Object.assign(this, _Config.defaults, env);
    this.#normalizeAll();
  }
  #normalizeAll() {
    ...
    this.proxyList = _Config.parseProxyList(this["USER_PROXY"]);
  }
  normalizePath(p) {
...
var Logger = class _Logger {
  ...
  constructor(level = "info", timeZone = "0") {
    ...
    this.timeDriftMs = isNaN(offset) ? 0 : offset * 36e5;
    this.levelIdx = _Logger.#LOG_LEVELS[(level || "info").toLowerCase()] ?? _Logger.#LOG_LEVELS.info;
  }
  debug(...msg) {
    if (this.levelIdx <= _Logger.#LOG_LEVELS.debug) this.#log("[debug]", ...msg);
  }
  info(...msg) {
    if (this.levelIdx <= _Logger.#LOG_LEVELS.info) this.#log("[info ]", ...msg);
  }
  error(...msg) {
    if (this.levelIdx <= _Logger.#LOG_LEVELS.error) this.#log("[error]", ...msg);
  }
  #generateRandomId() {
...
var Crypto = class _Crypto {
  ...
  static #H = [3238371032, 914150663, 812702999, 4144912697, 4290775857, 1750603025, 1694076839, 3204075428];
  static #encoder = new TextEncoder();
  static sha224Hash(str) {
    if (typeof str !== "string") throw new TypeError("sha224Hash: input must be a string");
    const rotr = /* @__PURE__ */ __name((x, n) => x >>> n | x << 32 - n, "rotr");
    const msgBytes = _Crypto.#encoder.encode(str);
    ...
    for (let offset = 0; offset < buffer.length; offset += 64) {
      ...
      for (let i = 0; i < 64; i++) {
        ...
        const ch = e & f ^ ~e & g;
        const temp1 = hh + S1 + ch + _Crypto.#K[i] + w[i] >>> 0;
...
var UuidHelper = class {
  static {
    __name(this, "UuidHelper");
  }
...
var PaddingGenerator = class {
  static {
    __name(this, "PaddingGenerator");
  }
  constructor() {
    this.cache = /* @__PURE__ */ new Map();
    this.rangeCache = /* @__PURE__ */ new Map();
  }
  generate(rangeStr) {
...
var StreamManager = class _StreamManager {
  ...
  createQueuingStrategy(bufferSize) {
    ...
    _StreamManager.#strategyCache.set(highWaterMark, strategy);
    return strategy;
  }
...
var WebSocketClient = class {
  ...
  #closeWebSocket(code = 1e3, reason = "Normal Closure") {
  ...
  #handleMessage = /* @__PURE__ */ __name((event) => {
  ...
  #handleError = /* @__PURE__ */ __name((event) => {
  ...
  #handleClose = /* @__PURE__ */ __name((event) => {
  ...
  #setupStreams() {
    ...
    this.writable = new WritableStream({
      ...
      close: /* @__PURE__ */ __name(() => this.writing && this.#setDone("writing"), "close"),
...
var XhttpClient = class {
  static {
    __name(this, "XhttpClient");
  }
...
var DataRelay = class {
  ...
  async relay(client, remote, vls) {
    const up = this.#pump(client, remote, vls.data).catch((err) => this.#logError("upload", err)).finally(() => client.readingDone && client.readingDone());
    const firstResp = vls.resp && vls.resp.byteLength > 0 ? vls.resp : void 0;
  ...
  async pipe(src, dst, firstPacket) {
    ...
    const opt = src.signal ? { signal: src.signal } : void 0;
    try {
  ...
  async yield(src, dst, firstPacket) {
    ...
    const p = new Promise((res, rej) => {
      (firstPacket && firstPacket.byteLength ? writer.write(firstPacket) : Promise.resolve()).then(() => this.#yieldCopy(res, rej, reader, writer)).catch(rej);
    });
    p.finally(() => {
      reader.releaseLock();
      writer.releaseLock();
...
var ProtocolParser = class {
  static {
    __name(this, "ProtocolParser");
  }
  constructor(name) {
    this.name = name;
  ...
  parseHostname(header, idx, atype) {
    ...
    if (atype === ATYPE.IPV4) {
      ...
      hostname = Array.from({ length: 8 }).map((_, i) => {
...
var SselvProtocolParser = class extends ProtocolParser {
  static {
    __name(this, "SselvProtocolParser");
  }
  constructor() {
    super("cfsselv");
  }
  match(buf, config) {
    if (buf.length < 17 || buf[0] !== 0) return false;
...
var NajortProtocolParser = class extends ProtocolParser {
  static {
    __name(this, "NajortProtocolParser");
  }
  constructor() {
    super("cfnajort");
  }
  match(buf, config) {
    ...
    return buf[56] === 13 && buf[57] === 10 && /^[A-Za-z0-9+/=]+$/.test(bufPassword) && bufPassword === config.T_SHA224PWD && buf[58] === 1 && [1, 3, 4].includes(buf[59]);
  }
  async parse(reader, state) {
    const PASS_LEN = 56;
...
var SkcoswodahsProtocolParser = class extends ProtocolParser {
  static {
    __name(this, "SkcoswodahsProtocolParser");
  }
  constructor() {
    super("cfskcoswodahs");
  }
  ...
  async parse(reader, state) {
    ...
    if (atype === 1) {
      addressLen = 4;
      await this.readUntil(reader, 1 + 4 + 2, state);
    } else if (atype === 4) {
      addressLen = 16;
      ...
      extra = 1;
      addressLen = len;
      await this.readUntil(reader, 2 + len + 2, state);
    } else {
...
var LinkGenerator = class _LinkGenerator {
  static {
    __name(this, "LinkGenerator");
  }
  static PARAMS = {
  ...
  #generateLinks(host, pathname, type) {
    const isWs = type === "ws";
    const sselvParams = isWs ? _LinkGenerator.PARAMS.SSELV_WS : _LinkGenerator.PARAMS.SSELV_XHTTP;
    const najortParams = isWs ? _LinkGenerator.PARAMS.NAJORT_WS : _LinkGenerator.PARAMS.NAJORT_XHTTP;
    ...
    const links = this.config.PREFERRED_ADDRESS.flatMap((addr) => {
      ...
      if (isWs) output.push(`${atob("c3M6Ly8=")}${btoa(`none:${this.config.V_UUID4}`)}@${addr}:${third}?${_LinkGenerator.PARAMS.SKCOSWODAHS_WS}${encodeURIComponent(`host=${host};path=${pathname}`)}#${atob("c2hhZG93c29ja3Mt")}${type}-tls`);
      return output;
    });
    return links.join("\n");
  }
...
var IPInfoProvider = class {
  static {
    __name(this, "IPInfoProvider");
  }
  getInfo(request) {
    const cf = request.cf || {};
    return {
...
var ClientProcessor = class {
  static {
    __name(this, "ClientProcessor");
  }
